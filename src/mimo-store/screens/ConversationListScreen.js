...
class ConversationItem extends React.Component {
  ...
  render() {
    ...
    // User name and photo.
    const user = stateManager.getUser(conversation.customer);
    let userName = '';
    let userPhoto = null;
    ...
    const userStyle = read ? styles.conversation.readUser :
      styles.conversation.unreadUser;
      ...
      <TouchableOpacity onPress={this._onPress}>
        <View style={containerStyle}>
          ...
          <View style={styles.conversation.textView}>
            <Text numberOfLines={1} style={userStyle}>
              {userName}
            </Text>
...
export default class ConversationListScreen extends React.Component {
  ...
  _filterByName = (data) => {
    let filteredData = data.filter((conversation) => {
      ...
      return userName.match(new RegExp('.*' + this.searchText + '.*', 'gi'));
    });
        ...
        <View style={styles.layout.list} >
          {conversations.length > 0 ?
            ...
            <Text style={styles.layout.placeholderText}>
              Você ainda não tem conversas com consumidores.
            </Text>
          }
        </View>
      </View>
